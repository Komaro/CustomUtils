...
public partial class EditorBuildServiceEx : EditorService {

    private static EditorWindow _window;
    private static EditorWindow Window => _window == null ? _window = GetWindow<EditorBuildServiceEx>("Build Service") : _window;
    ...
    private static readonly string SELECT_DRAWER_KEY = $"{nameof(EditorBuildServiceEx)}_{nameof(EditorDrawer)}";
    ...
    public static void CacheRefresh() {
        if (HasOpenInstances<EditorBuildServiceEx>()) {
            _builderTypes = ReflectionProvider.GetSubClassTypes<BuilderEx>().OrderBy(type => type.TryGetCustomAttribute<PriorityAttribute>(out var attribute) ? attribute.priority : 99999).ToArray();
            if (_builderTypes.Any()) {
                _builderTypeNames = _builderTypes.Select(type => type.Name).ToArray();
            }
    ...
    private void OnGUI() {
        ...
        if (_selectBuilderIndex < _builderTypes.Length && _selectBuilderType != _builderTypes[_selectBuilderIndex]) {
            DrawerClose();
            _selectBuilderType = _builderTypes[_selectBuilderIndex];
            EditorCommon.Set(SELECT_DRAWER_KEY, _selectBuilderType.Name);
            DrawerCacheRefresh();
        }
