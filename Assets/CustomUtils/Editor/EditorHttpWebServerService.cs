...
public class EditorHttpWebServerService : EditorService<EditorHttpWebServerService> {
    
    private SimpleHttpServer _httpServer;
    private string[] _hosts;
    private List<Type> _serveModuleList = new();

    private Config _config;
    private SerializedStringArray _prefixes;
    ...
    // TODO. 임시 처리. 향후 통합 처리를 위한 구조 추가 후 구조적 개선 예정
    private string CONFIG_PATH = Path.Combine(Constants.Path.COMMON_CONFIG_PATH, CONFIG_NAME);
    ...
    protected override void OnEnable() {
        // TODO. 임시 처리. 향후 통합 처리를 위한 구조 추가 후 구조적 개선 예정
        CONFIG_PATH ??= Path.Combine(Constants.Path.COMMON_CONFIG_PATH, CONFIG_NAME);
        base.OnEnable();
    }
    ...
    protected override void Refresh() {
        if (HasOpenInstances()) {
            ...
            _hosts = Dns.GetHostEntry(Dns.GetHostName()).AddressList.Where(address => address.AddressFamily == AddressFamily.InterNetwork).Select(address => address.ToString()).ToArray();
            _serveModuleList = ReflectionProvider.GetSubTypesOfType<HttpServeModule>().ToList();

            _prefixes = CreateInstance<SerializedStringArray>();
            _prefixes.Init(_config.urls);
        }
    }
    ...
    private void OnGUI() {
        ...
        if (_config != null) {
            ...
            using (new EditorGUILayout.VerticalScope(Constants.Draw.BOX)) {
                if (_hosts?.Length > 0) {
                    GUILayout.Label("Current IP Addresses", Constants.Draw.AREA_TITLE_STYLE);
                    foreach (var address in _hosts) {
                ...
                using (var changeScope = new EditorGUI.ChangeCheckScope()) {
                    GUILayout.Label("URLs", Constants.Draw.AREA_TITLE_STYLE);
                    _prefixes.Draw();

                    if (changeScope.changed) {
                        _config.urls = _prefixes.Value;
                        _config.SetDirty();
                    }
                }
            }
            ...
            using (new GUILayout.VerticalScope()) {
                ...
                if (_httpServer != null && GUILayout.Button("서버 제거", GUILayout.Height(35))) {
                    _httpServer.Close();
                    _httpServer = null;
                }
    ...
    private void DrawServeModulePreset() {
        ...
        using (new GUILayout.VerticalScope(Constants.Draw.BOX, GUILayout.ExpandHeight(false))) {
            ...
            if (_httpServer != null) {
                ...
                foreach (var type in _httpServer.GetServeModuleTypes) {
        ...
        using (new GUILayout.VerticalScope()) {
            ...
            foreach (var type in _serveModuleList) {
                if (_httpServer == null) {
                    ...
                    if (GUILayout.Button($"{type.Name} {(alreadyModule ? "제거" : "추가")}") && _httpServer != null) {
                        if (alreadyModule) {
    ...
    private void StartHttpWebServer() {
        foreach (var url in _config.urls) {
            if (URL_REGEX.IsMatch(url) == false) {
                EditorCommon.ShowCheckDialogue("경고", $"유효하지 않은 URL 주소가 확인됩니다.\n{url}\n주소는 아래와 같이 시작하여야 합니다.\nhttp://, https://, ftp://");
                return;
            }
        }
        
        _httpServer ??= new SimpleHttpServer(_config.urls);
        if (_httpServer.IsRunning()) {
            EditorCommon.ShowCheckDialogue("경고", "현재 임시 웹 서버가 동작중입니다.\n확인 시 종료 후 서버를 재시작합니다.", ok: () => { _httpServer.Restart(); });
        } else {
            _httpServer.Start(_config.targetDirectory);
        }
    }

    private class Config : JsonCoroutineAutoConfig
    {
        public string targetDirectory = "";
        public string[] urls = { "http://localhost:8000/" };
