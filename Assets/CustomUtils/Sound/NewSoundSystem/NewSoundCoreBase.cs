...
namespace CustomUtils.Sound.NewSoundSystem {
    ...
    public abstract class SoundCoreBase : MonoBehaviour, IDisposable {
        ...
        [RefactoringRequired]
        [TestRequired]
        protected virtual ImmutableDictionary<Enum, SoundType> LoadSoundTypeDic() => GetEnums<MasterSoundEnumTypeAttribute>()
            .ToDictionary(enumValue => enumValue, enumValue => new MasterType(enumValue) as SoundType)
            .Pipe(dictionary => dictionary.Concat(ReflectionProvider.GetAttributeEnumSets<ControlSoundEnumTypeAttribute>()
